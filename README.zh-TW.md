...
| 場景 | 建議 | 原因 |
|------|------|------|
| 簡單腳本、CLI 工具（無 asyncio） | **ws-rs sync** | picows 不能用；對 websocket-client 領先 30–60% |
| asyncio 應用內、序列 RR < 100 KB | **ws-rs sync**（從 `to_thread` 呼叫）或 **picows** | sync 贏 raw 吞吐；picows 贏單筆 RT 延遲 |
| asyncio 應用內、pipelined / streaming | **ws-rs async** | pipelined 吞吐領先 picows 9–21% |
| asyncio 應用內、≥ 1 MB payload | **ws-rs async** ≈ **picows** | 大致並列；都撞 server 加密天花板 |
| HFT / 同機房交易、單筆 RT µs 級敏感 | **picows** | RR 延遲低 3–10 µs；Cython 直 call CPython API |
| 跨網路（真實 server） | **任何** | 網路 RT 主導，lib 選擇 < 1% 差 |

> **vs picows 總結**：ws-rs 贏在**穩態吞吐**（尤其 pipelined）並提供 picows 沒有的 **sync API**；picows 贏在**單筆 RT 延遲** 3–10 µs（PyO3 vs Cython 架構固有差），只在 colo HFT 真有意義。
