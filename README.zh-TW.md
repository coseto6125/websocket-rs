...
### Request-Response 吞吐量（picows-parity benchmark，10 秒，RPS）

採用 picows 官方 benchmark 方法論 — RR 模式（send → wait → recv → 重複），中立 Rust echo server（tokio-tungstenite）、綁定 CPU core、uvloop，每格 10 秒 N≈120k–2.6k：

| Payload | **websocket-rs** | picows | aiohttp | websockets | websocket-client |
|---------|---:|---:|---:|---:|---:|
| 256 B   | **13.9k** | 12.8k | 11.8k | 9.4k  | 11.5k |
| 8 KB    | **13.4k** | 12.8k | 10.8k | 9.7k  | 9.7k  |
| 100 KB  | **9.8k**  | 9.3k  | 8.7k  | 7.5k  | 4.4k  |
| 2 MB    | **2.6k**  | 2.4k  | 2.2k  | 1.9k  | 268   |

> websocket-rs 在每個 payload 大小都領先。相較 picows 領先 2–8%；相較 websockets/aiohttp 領先 20–50%；相較 websocket-client 在 ≥100 KB 時快 3–10×。
>
> 已在三種 server 架構驗證（tokio-tungstenite、fastwebsockets、picows-server）— 12 格矩陣全部成立。實際網路環境下，各函式庫的差異 < 1% 總 RTT（網路延遲主導）。

📊 **[完整 benchmarks — 所有尺寸、所有 server、pipelined 模式、我們落敗的案例](docs/BENCHMARKS.md)** | 📝 **[最佳化研究](docs/OPTIMIZATION_RESEARCH.md)**
...
# 執行 picows-parity RPS benchmark（5 clients × 3 server 架構 × 4 尺寸，約 10 分鐘）
