...
採用 picows 官方 benchmark 方法論 — RR 模式（send → wait → recv → 重複），中立 Rust echo server（tokio-tungstenite）、綁定 CPU core、uvloop，每格 10 秒 N≈260–14.8k：
...
> websocket-rs 在三種 server 架構共 **24 格全部領先**。Sync API 在 256 B–100 KB 勝出（無 asyncio overhead）；Async 在 2 MB 與 picows 並列。相較 picows 領先 2–18%；相較 websockets/aiohttp 領先 15–60%；相較 websocket-client 在 ≥100 KB 時快 2–10×。
